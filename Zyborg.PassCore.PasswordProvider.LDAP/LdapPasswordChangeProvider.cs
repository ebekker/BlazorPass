    ...
    public class LdapPasswordChangeProvider : IPasswordChangeProvider
    {
        ...
        private void Init()
        {
                ...
                params (bool throwIfFail, bool isFail, string failMsg)[] validations)
            {
                ...
                {
                    if (check.isFail)
                        if (check.throwIfFail)
                            throw new ArgumentException(check.failMsg, name);
                        else
                            _logger.LogWarning($"option [{name}] {check.failMsg}");
                }
                ...
                if (value == null)
                    ...
                    printValue = JsonConvert.SerializeObject(value);
                _logger.LogInformation($"  * [{name}] = [{printValue}]");
            }
        }
        ...
        private LdapConnection BindToLdap()
        {
            ...
            ldap.Bind(_options.LdapBindDistName, _options.LdapBindPassword);
