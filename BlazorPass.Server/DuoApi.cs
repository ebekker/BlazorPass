...
namespace Duo
{
    public class DuoApi
    {
            ...
            where T : class
        {
            ...
            return JSONDecodeCall<T>(res, statusCode);
        }

        public T JSONDecodeCall<T>(string res, HttpStatusCode statusCode)
            where T : class
        {
            try
            {
                var dict = JObject.Parse(res);
                if (dict.Value<string>("stat") == "OK")
                {
                    return dict.GetValue("response").ToObject<T>();
                }
                else
                {
                    var check = dict.Value<int?>("code");
                    var code = check ?? 0;
                    var message_detail = dict.Value<string>("message_detail") ?? string.Empty;
                    throw new ApiException(code,
                                           (int)statusCode,
                                           dict.Value<string>("message"),
                                           message_detail);
                }
            }
            catch (ApiException)
